...
//            var dt_dt = dt[0].split("-");
            ...
            if(dataatual > datafim){
				// não pode votar
                var status = 0;
				$("#enquete #enquete_periodo" ).css('color','red');
                //alert('0');
            }else{
                var status = 1;
                //alert('0');
			...
			if($( "#DADOS #PARENTESCO" ).val() == 1 && retorno[0]['voto'] == null){
				$(".resposta_enquete_lista_father").hide();
				if(status == 0){
					...
					$( "#enquete .enquete_votos_total" ).hide();
			}else{
				carrega_resposta(id);	
				$(".resposta_enquete_lista_father").show();
				resposta_morador(id);
			    $( "#enquete .enquete_votos_total" ).show();
				setTimeout(function(){
		...
		success: function(retorno){
            ...
            for (x in retorno) {
                var dado = '<li><label class="item-radio item-content">'				
						...
						+'<div class="item-title">'+retorno[x]['pergunta']+'</div>'
					+'</div>'
					+'</label></li>';
                dados = dados + dado;
            }
             var bt_submit = '<button type="button" class="btn btn-success" name="votar" onclick="atualiza_enquete('+id+');">Votar</button>';
            
                dados = dados +'</ul></div>'+ bt_submit;
...
let resposta_morador = (id) => {
	
	let x                     = 0;
	let html                  = '';
	let id_usuario_condominio = $('#DADOS #ID_USER').val();
	$("#resposta_enquete_lista>div").remove('');
	$.ajax({
		type: 'POST',
        url        : localStorage.getItem('DOMINIO')+"appweb/enquete_get.php",
		crossDomain: true,
		...
		success: function(retorno){
		    ...
		    $("#resposta_enquete_lista a").after(html); 
	...
	$.ajax({
		...
		success: function(retorno){
            ...
            for (x in retorno) {
				...
				cor[cor.length] = matrizCor(qt_dados);
