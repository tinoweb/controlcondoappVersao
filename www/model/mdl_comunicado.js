...
function carrega_comunicados(tipo){
	...
	$.ajax({
		...
		crossDomain: true,
		beforeSend : function() { $("#skul-comunicado").css("display", "block"); },
		complete   : function() { $("#skul-comunicado").css("display", "none"); },
		...
		success: function(retorno){
			...
			if (cont == 0){
				var sem_reg = "<div align='center' style='margin-top: 50%; width:100%'><h4>Ops! Nenhum registro encontrado aqui :(</h4><br></div>";
				
			}
		
			setTimeout(function(){$( "#main_comunicado" ).append(dados)},300);
...
function carrega_comunicado(id){
	...
	$.ajax({
		...
		success: function(retorno){
			...
			$( "#titulo_comunicado_view" ).html(retorno[0]['titulo']);
